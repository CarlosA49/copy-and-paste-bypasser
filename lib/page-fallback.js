...
(function (root) {
  ...
  function findGradedResultsIndicator(doc) {
    ...
    // Post-submission-only signals. Deliberately exclude "your grade" / generic
    // grade panels — those appear on retake pages for a PRIOR attempt and would
    // falsely confirm before the current attempt is submitted.
    const RE = /\b(grade received|submission received)\b/i;
    const nodes = doc.querySelectorAll('h1, h2, h3, [role="heading"], [role="status"], [data-testid*="submission" i]');
