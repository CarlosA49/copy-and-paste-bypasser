...
(function (root) {
  ...
  // Legacy equals-based extraction. Retained so that tests depending on the
  // "= VALUE [UNIT]" pattern continue to pass. Tasks B2/B3 will retrofit this
  // with confidence/label fields; for now objects are returned without them so
  // that deepEqual assertions expecting confidence:'low' remain failing.
  ...
  // Low-confidence pass: last "= VALUE [UNIT]" in the text (fallback for worked
  // solutions). Returns a plain {value, unit, raw} object — confidence/label
  // fields are intentionally absent until Task B3 retrofits this pass.
  function extractEqualsPass(rawText) {
    ...
    if (!lastEquals) return null;
    const value = lastEquals[1];
    let u = lastEquals[2] || '';
    if (u && STOP_UNITS.has(u.toLowerCase())) u = '';
    const raw = u ? (value + ' ' + u) : value;
    return { value: value, unit: u, raw: raw };
  }
  ...
  // and low-confidence passes; for now we have the high-confidence labeled pass
  // plus the legacy equals pass (without confidence fields, pending B3).
  function extractAnswers(rawText) {
    ...
    // Merge labeled (high confidence) with the legacy equals pass.
    // The equals pass is only used when it finds a value not already captured
    // by the labeled pass, and when there are no labeled results already covering
    // the same value.
    const combined = labeled.slice();
    const eq = extractEqualsPass(normalised);
    if (eq) {
      const key = eq.value + '|' + eq.unit.toLowerCase();
      const alreadyCovered = combined.some(function (a) {
        return (a.value + '|' + (a.unit || '').toLowerCase()) === key;
      });
      if (!alreadyCovered) {
        combined.push(eq);
      }
    }
    return dedupAnswers(combined);
  }
