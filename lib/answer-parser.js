...
(function (root) {
  ...
  const STOP_UNITS = new Set([
    ...
    'note','next','here','just','also','only','both','once','now','see',
  ]);
  ...
  const LABEL_PATTERNS = [
    ...
    // "Question 1: ...", "Question 1 ...", "question 2, use ..."
    // Allow a comma as separator and an optional single-word bridge ("use", "gives", etc.)
    // before the value so "question 2, use 0.0352 H" is captured.
    new RegExp('\\bquestion\\s+(\\d{1,2})\\s*[:.=,]?\\s*(?:[A-Za-z]+\\s+){0,3}(?:is\\s+)?' + VALUE_RE + '(?:\\s*' + UNIT_RE + ')?', 'gi'),
    // "Answer 2 = ...", "Answer 2: ..."
    ...
    // "result is X" / "outcome is X" / "value is X" (no question label).
    // m.length === 3: (value, unit) — uses the no-label path in extractLabeledAnswers.
  ...
  // Require that "." as a list separator is followed by whitespace (not a digit),
  // so "1.0000e-6 H" is not mis-parsed as list item 1 with value "0000e-6".
  const NUMBERED_LINE_RE = new RegExp(
    '^\\s*(\\d{1,2})(?:[)]\\s*|\\.\\s+|\\s)\\s*' + VALUE_RE + '(?:\\s*' + UNIT_RE + ')?',
    'gm'
  );
  ...
  // A line whose ENTIRE content is just "VALUE [UNIT]" — no label, no equation.
  // This catches split scientific-notation inputs that normalise to a bare value
  // on a single line (e.g. "1.0000×10\n−6\nH" → "1.0000e-6 H"). Confidence: medium.
  // Note: separator between value and unit is [ \t]+ (no newline) so the
  // pattern cannot bridge across a line break and grab the next line as a
  // "unit". The value-and-unit must be on the same physical line.
  const BARE_VALUE_LINE_RE = new RegExp(
    '^[ \\t]*' + VALUE_RE + '(?:[ \\t]+' + UNIT_RE + ')?[ \\t]*$',
    'gm'
  );

  // A bare integer with no unit and no decimal is almost always a stray digit
  // mid-derivation (e.g. "Step 3" or an enumeration). Such matches are demoted
  // to 'low' confidence so dedup prefers any labeled/list/inline answer above.
  function looksAnswerShaped(value, unit) {
    if (unit) return true;                  // value+unit on its own line — kept
    if (/[.e]/.test(value)) return true;    // decimal or scientific — kept
    return false;                           // bare short integer — demoted
  }

  function extractBareValueLines(normalisedText) {
    const out = [];
    BARE_VALUE_LINE_RE.lastIndex = 0;
    let m;
    while ((m = BARE_VALUE_LINE_RE.exec(normalisedText)) !== null) {
      const value = m[1];
      const unit = m[2] || '';
      if (!value) continue;
      const confidence = looksAnswerShaped(value, unit) ? 'medium' : 'low';
      out.push(makeAnswer(value, unit, confidence, null));
    }
    return out;
  }
  ...
  function extractAnswers(rawText) {
    ...
    const bare = extractBareValueLines(normalised);
    return dedupAnswers(labeled.concat(listed).concat(inline).concat(bare));
  }
  ...
  const api = { parseAnswerText: parseAnswerText, cleanFillValue: cleanFillValue, MAX_QUESTION_NUMBER: MAX_QUESTION_NUMBER };
