...
(function (root) {
  ...
  function wireAnswer() {
    ...
    apply.addEventListener('click', function () {
      ...
      // Fallback shared by both "no candidates" and "no fills" paths: route the
      // raw paste through the question-indexed applier. Handles "N bare lines
      // for N questions" pastes (with or without preamble) that the legacy
      // candidate-parser doesn't recognize.
      function applierFallback() {
        const ccp = window.ClipboardCleaner || {};
        const applier = ccp.answerApplier;
        if (!applier || typeof applier.applyAnswers !== 'function') return null;
        try { return applier.applyAnswers(raw, document.body, { verbose: false }); }
        catch (_) { return null; }
      }

      let optionSummary = { selected: 0, skipped: 0 };
      let textSummary = { filled: 0, skipped: 0 };
      let textInputs = [];

      if (hasAnyCandidate) {
        const groups = matcher.findOptionGroups(document.body);
        const matches = (groups.length > 0) ? matcher.matchCandidates(groups, parsed) : [];
        optionSummary = (matches.length > 0) ? matcher.applyMatches(matches) : optionSummary;

        textInputs = matcher.findTextInputs ? matcher.findTextInputs(document.body) : [];
        const textMatches = matcher.matchTextInputs
          ? matcher.matchTextInputs(textInputs, parsed)
          : [];
        textSummary = (textMatches.length > 0 && matcher.applyTextMatches)
      ...
      if (did === 0) {
        const fb = applierFallback();
        const fbFilled = (fb && fb.summary && fb.summary.filled) || 0;
        if (fbFilled > 0) {
          const fbFailed = (fb.summary && fb.summary.failed) || 0;
          const fbParts = ['Filled ' + fbFilled];
          if (fbFailed) fbParts.push('(' + fbFailed + ' skipped)');
          setAnswerStatus(fbParts.join(' '), 'success');
          return;
        }
        setAnswerStatus(
          hasAnyCandidate
            ? 'No options matched the parsed answer, and no text fields could be filled.'
            : 'No answer candidates found in the pasted text.',
          'error'
        );
        return;
      }
  ...
  function wireAutopilot() {
    ...
    function emitSettings() {
      ...
      if (_autopilotHandlers.onSettingsChange) {
        _autopilotHandlers.onSettingsChange({
          ...
          aiAnswerAssessments: !!(aia && aia.checked),
    ...
    if (aia) aia.addEventListener('change', emitSettings);
