...
(function (root) {
  ...
  function isCompleted(anchor) {
    for (let i = 0; i < COMPLETED_SELECTORS.length; i++) {
      ...
      if (!el) continue;
      // Exclude elements whose aria-label explicitly negates completion
      // e.g. aria-label="Not completed" also contains "completed"
      const aria = el.getAttribute && el.getAttribute('aria-label');
      if (aria && /\bnot\b/i.test(aria)) continue;
      return true;
    }
    return false;
  }
