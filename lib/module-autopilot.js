...
(function (root) {
  ...
  function createAutopilot(opts) {
    ...
    async function start(opts) {
      ...
      // Only kick the handler immediately from start() when we navigated to the
      // first queue item (cursor=0). When startCursor > 0, the user was already
      // on that item and the SPA route-change handler (bootIfRunning) takes over.
      // For cursor=0: if the current URL already matches queue[0], no SPA event
      // fires so we kick the handler directly.
      if (startCursor === 0) {
        const curItemId = scraperMod.extractItemId(currentUrl());
        if (curItemId && curItemId === safeQueue[0].id) {
