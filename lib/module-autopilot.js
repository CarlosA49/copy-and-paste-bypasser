...
(function (root) {
  ...
  const FAILURE_REASON_TEXT = {
    ...
    'peer-review-needs-user': "Peer review needs you — complete it manually. I'll resume automatically once it's marked complete.",
  ...
  function formatPeerReviewLogLines(outcome) {
    ...
    if (outcome.outcome === 'peer-review-needs-user') {
      return ["⏸ Peer review needs you — complete it manually; I'll resume automatically once it's marked complete"];
    }
  ...
  function createAutopilot(opts) {
    ...
    const aiGenerate = (typeof opts.aiGenerate === 'function') ? opts.aiGenerate : null;
    ...
    function runCurrentItem(stateNow) {
      ...
      inFlight = (async function () {
        ...
        if (alreadyCompleteIndicator) {
          ...
          if (!(outcome && outcome._aiSkip) && !(outcome && outcome.outcome === 'peer-review-submitted') && confirmer && typeof confirmer.waitForCompletion === 'function') {
        ...
        // AI-skip items emit only the ⏭ Skipped line (above); suppress the ✓ here.
        if (!alreadyCompleteIndicator && !(outcome && outcome._aiSkip) && !(outcome && outcome.outcome === 'peer-review-submitted')) {
  ...
  const api = {
    ...
    buildOrderedQueue: buildOrderedQueue,
