...
(function (root) {
  ...
  // it on plain words ("one" -> "on*e", multi-word phrases truncated) corrupts text.
  ...
  function applyAnswers(rawAnswerText, rootEl, options) {
    ...
    questions.forEach(function (q) {
      ...
      // Option-enumeration lines (e.g. "A: X B: Y C: Z") provide no actionable
      // selection — refuse rather than guess (clean miss > wrong fill).
      if (p.enumeration) {
        results.push({ questionNumber: q.questionNumber, type: q.type, status: 'failed', reason: 'option-enumeration', rawAnswer: p.rawAnswer });
        if (verbose) console.log('  Q' + q.questionNumber + ' ' + q.type + ' FAIL option-enumeration');
        return;
      }

      // free_text/code are textarea/contenteditable-backed (question-detector now
      // classifies a lone textarea as free_text and a code-editor surface as code).
      // They fill exactly like the legacy math_input text path here — the rawText
      // manual-paste applier is type-agnostic about how it fills q.targets — so
      // routing them through this branch preserves the prior fill behavior.
      if (q.type === 'math_input' || q.type === 'numerical' || q.type === 'input'
          || q.type === 'free_text' || q.type === 'code') {
        ...
        if (p.segments && q.targets.length > 1) {
          ...
          for (let j = 0; j < n; j++) {
            ...
            // Use the segment value directly: these are fill-in-the-blank text fields
            // (words, short phrases, or simple numbers) — math normalization corrupts
            // plain words (e.g. "one" → "on*e") and is not needed here.
            const valNorm = (segs[j].value || '').trim();
        ...
        // For textarea targets, use the raw answer directly (prose pass-through) rather than
        // running through math normalization which collapses whitespace and inserts operators.
